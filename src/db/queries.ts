...
export async function getLastSessionLogs(
  ...
  if (last.session_id) {
    const rows = await db.prayer_logs
      .where('session_id')
      .equals(last.session_id)
      .toArray();
    return rows.map((r) => ({ id: r.id!, prayer: r.prayer, quantity: r.quantity }));
  }
...
export async function getStats(db: QadaDB): Promise<StatsState> {
  ...
  const totalRemaining = PRAYER_NAMES.reduce(
    (sum, p) => sum + (debts[p]?.remaining ?? 0),
    0,
  );
